...
Applies to `frontend/apps/h5`, `frontend/apps/packages/domain`, `frontend/apps/packages/api-client`, and `frontend/apps/packages/hooks`. H5 is the current primary debugging target unless the user explicitly asks for Taro miniapp or admin.

## React/H5 Rules

- Keep pages in `frontend/apps/h5/src/pages/<Module>/index.tsx`; each page imports `index.less`. Page-private components live as flat files under `pages/<Module>/components/*.tsx`, and their styles live in the page `index.less`.
- Keep H5 home-domain pages under `frontend/apps/h5/src/pages/home/<domain>` and keep each domain's private components, hooks, and model in that same domain folder. Do not scatter a domain's sibling pages at the `pages` root.
...
- Keep reusable pure functions in `frontend/apps/h5/src/tools/*.ts`.
...
- Keep shared/global hooks in `frontend/apps/h5/src/hooks`; keep page or business-module local hooks in `pages/<Module>/hooks/*.ts` instead of next to the page component file.
...
- Keep API types in `frontend/apps/packages/domain`. Update `api-client` and hooks together when backend contracts change.
...
Run `npm run typecheck:h5` and `npm run lint:h5` from `frontend/apps`. Existing `MessageToast` Fast Refresh warnings may be reported as known warnings unless the task is about that component.
