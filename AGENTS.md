...
`scripts/dev/h5.sh`/`scripts/dev/app-server.sh`/`scripts/dev/h5+app-server.sh` 启动前会自动检查 `8899`/`9988` 是否已有监听，若已监听会先尝试 `kill` 掉占用端口的旧进程（等待端口释放，超时后升级为 `kill -9`），再启动新进程，不再是"已监听就跳过启动"。`15432` 是数据库隧道端口，仍按已有监听优先复用处理。这几个脚本只应由用户手动运行，Claude/Codex 不主动执行（见"验证命令"一节）。
...
- H5 首页业务域统一收敛在 `pages/home` 下：委托/狩猎归属 `home/commission`，兼职归属 `home/job`，家教归属 `home/edu`，优选/商品归属 `home/shop`；同域组件、hooks、model 不得散落到其他页面目录。
...
- 本地 H5/后端 dev server 的启动、重启、停止一律由用户手动执行；不主动 kill 已有进程，也不主动执行 `scripts/dev/h5.sh`、`scripts/dev/app-server.sh`、`scripts/dev/h5+app-server.sh` 或等价的启动/重启命令。
- 已确认需要重启后端才能生效的场景（例如 Java 代码改动，无热重载），只说明"哪些改动需要重启才能生效"和建议的重启方式，交由用户执行；重启完成后再按用户确认的时机继续验证健康检查或目标接口。
- 如果本次修改不涉及后端代码、配置、数据库迁移或后端运行时依赖，不需要重新启动后端服务，直接复用已有后端进程做接口或页面验证。
- 默认端口无监听、或确认需要新起本地服务时，先向用户说明原因，由用户决定是否及何时启动，不擅自启动新的本地服务。
