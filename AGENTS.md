...
- 注释要求规范同步适用于后端项目；后端新增或维护 Java 代码时，类、控制器、应用服务、请求/响应模型、异常、配置、状态常量和关键公共方法必须补充 JavaDoc。
...
- 项目开发阶段所有业务流转必须走真实后端接口并连接真实数据库；涉及状态变化、权限动作、按钮显隐、流程节点、当前使用项、申请/确认/取消/完成等业务判断时，必须先确认或补齐服务端状态机、数据库字段/记录、接口响应和接口动作，再接入 H5 展示与交互。
- 不得把业务流转实现为 H5 本地状态、本地缓存、纯 toast、前端假按钮显隐或 mock 数据；发现现有实现仍停留在本地流转时，必须优先改为后端闭环，再继续做页面组件调整。
...
- H5 只能消费后端返回的真实业务状态并触发真实接口动作；凡是影响业务流程、状态推进、角色权限、操作按钮可见性或数据归属的逻辑，不得只在前端用派生状态、临时字段或本地事件模拟，必须回到后端接口和数据库确认。
...
- `components/<Component>` 下的共享组件按文件夹维护，组件专属样式必须放在同目录 `index.less` 并由组件入口导入；`styles.less` 只保留全局基础、通用按钮、通用布局和确实跨组件共享的样式。
- 与页面业务无关、可复用的纯计算工具应放入 `client/apps/h5/src/tools/*.ts`，例如金额格式化、月份计算、日期 key 生成、日历任务映射和字段校验；页面与组件内不保留可复用工具函数。
- 页面组件负责业务编排，通用组件负责展示和自身交互闭环；组件需要数据时通过 props 传入，不直接读取全局用户资料或自行拼装业务数据。
- 组件 props 设计优先减少父级透传噪音：纯展示反馈、按钮事件包装、表单临时草稿等可由组件或页面内部独立完成的逻辑，应收敛到对应组件或页面内；父级只接收最终数据或业务动作回调。
- 可配置项存在稳定默认值时应在组件内部提供默认值，调用方不得重复传入默认值，例如 `AddressInfoForm` 默认 `mode="edit"`。
- 预览卡片类组件如需对当前条目执行编辑、删除、设为当前等操作，应由组件内部绑定当前条目并调用 `onEdit(item)`、`onDelete(item)`、`onUse(item)` 等回调，避免父级在列表渲染时重复包一层匿名函数。
...
- 地址信息表单必须复用 `client/apps/h5/src/components/AddressInfoForm`，并同时支持编辑模式、列表预览模式和紧凑卡片预览模式；编辑模式由组件内部维护输入草稿并向父级回传完整 draft，卡片预览需要展示当前使用状态 tag 和编辑入口。
...
- Hook 使用应靠近真实消费方：不为了向子组件或孙组件传递状态而在父组件提前调用 hook 后层层透传；子组件需要某个状态、数据或动作时，优先在该组件内部直接调用对应 hook。
- 全局数据或状态必须通过根节点 Provider/Context 提供，例如 `GlobalStoreProvider`；子组件和孙组件直接通过 Context/store hook 获取全局状态，不通过页面层重复传递全局对象。
- 全局通用 hook 维护在 `client/apps/h5/src/hooks`；页面或业务模块私有 hook 维护在对应模块的 `hooks` 目录，例如 `client/apps/h5/src/pages/Delegation/hooks/*.ts`，不得与页面组件文件同级散落。
...
- 全 H5 或全仓库范围的规范治理、目录迁移和批量重构必须分批次执行；每批先明确范围和问题分类，完成该批修改后运行对应验证命令，再继续下一批，避免一次性大改导致问题难以定位。
- 删除或拆分组件目录前，必须先用 `rg` 检查显式 import、JSX 使用、`auto-imports.d.ts` 和 auto-import 配置；被 auto-import 暴露的导出即使没有显式 import，也可能正在运行路径中使用。
- 命名为集合的组件目录（如 `AppShell`）如果仅用于汇总多个外壳组件，应优先拆分为职责明确的组件目录；拆分时必须同步调整引用、样式归属和 auto-import 声明。
